...
if __name__ == "__main__":
    # clear output dir
    if os.path.exists(os.path.join("results", "eight_rooms_bc")):
        shutil.rmtree(os.path.join("results", "eight_rooms_bc"))

    ray.init(temp_dir='/tmp/ray-eight-bc', log_to_driver=False)
    print("Starting Runs...")
    tune.run(run, config=config, local_dir="results/", name="eight_rooms_bc", resources_per_trial={'cpu': 1})
    print("Finished!")
