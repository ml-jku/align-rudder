...
if __name__ == "__main__":
    # clear output dir
    if os.path.exists(os.path.join("results", "four_rooms_alignrudder")):
        shutil.rmtree(os.path.join("results", "four_rooms_alignrudder"))

    ray.init(temp_dir='/tmp/ray-four-align', log_to_driver=False)
    print("Starting Runs...")
    tune.run(run, config=config, local_dir="results", name="four_rooms_alignrudder")
    print("Finished!")
